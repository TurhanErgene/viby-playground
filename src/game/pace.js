...
import { WEATHER, BUDGET } from './balance.js';
...
export function effectiveMu(stats, surfaceId, weatherKey, bank = 0) {
  ...
  const roughLoss = surf.rough * Math.pow(1 - stats.absorption, 1.2) * 0.95;
...
export function speedProfile(track, stats, weatherKey = 'clear', skill = 1) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const bog = Math.min(0.52, surf.drag * 0.085 * Math.pow(1 - stats.absorption, 1.15));
...
export function estimateLapTime(track, stats, weatherKey = 'clear', skill = 1) {
  ...
  const reach = 1 - BUDGET.skill * 0.70 * (1 - skill);   // lines, apexes, commitment
  let t = 0;
  ...
  const boostGain = 1 - BUDGET.skill * 0.52 * skill;
  return t * boostGain;
}
